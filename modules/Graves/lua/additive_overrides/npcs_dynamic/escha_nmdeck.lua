...
m:addOverride("xi.zones.Escha_RuAun.Zone.onInitialize", function(zone)
    ...
    local suzport = zone:insertDynamicEntity({
        ...
        onTrigger = function(player, npc)
            if player:getGMLevel() > 0 and player:checkNameFlags(0x04000000) or
                player:getCharVar("[SKY2]Suzaku_POP") == 1 then
    ...
    local byaport = zone:insertDynamicEntity({
        ...
        onTrigger = function(player, npc)
            if player:getGMLevel() > 0 and player:checkNameFlags(0x04000000) or
                player:getCharVar("[SKY2]Byakko_POP") == 1 then
    ...
    local genport = zone:insertDynamicEntity({
        ...
        onTrigger = function(player, npc)
            if player:getGMLevel() > 0 and player:checkNameFlags(0x04000000) or
                player:getCharVar("[SKY2]Genbu_POP") == 1 then
    ...
    local syrpop = zone:insertDynamicEntity({
        ...
        onTrigger = function(player, npc)
            if player:getGMLevel() > 0 and player:checkNameFlags(0x04000000) or
                player:getCharVar("[SKY2]Seiryu_POP") == 1 then
    ...
    local kirpop = zone:insertDynamicEntity({
        ...
        onTrigger = function(player, npc)
            if player:getGMLevel() > 0 and player:checkNameFlags(0x04000000) or player:getCharVar("[SKY2]Kirin_POP") == 1 then

            local menu =
            {
                title = "Ready to battle Kirin?",
                onStart = function(playerArg)
                    -- NOTE: This could be used to lock the player in place
                    playerArg:PrintToPlayer("By selecting \"Yes\" you will lose your key items for Byakko, Genbu, Seiryu and Suzaku.", xi.msg.channel.NS_SAY)
                end,
                options =
                {
                    {
                        "Yes, I'm ready!",
                        function(playerArg)
                            player:setCharVar("[SKY2]Kirin_POP", 0)
                            playerArg:PrintToPlayer("You can feel a thunderous force culminating from beneath you...", xi.msg.channel.NS_SAY)
                            playerArg:timer(3000, function(playerArg)
                                GetMobByID(mob.KIRIN):setSpawn(-0.769, -54.040, -600.119, 67)
                                SpawnMob(mob.KIRIN):updateClaim(playerArg)
                            end)
                        end,
                    },
                    {
                        "No, not yet.",
                        function(playerArg)
                            playerArg:PrintToPlayer("I'll be waiting...", xi.msg.channel.NS_SAY)
                            return
                        end,
                    },
                },
